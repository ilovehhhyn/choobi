...
def _build_parser() -> argparse.ArgumentParser:
    ...
    au.add_argument("runtime", nargs="?", choices=["claude", "codex"])
...
def _print_auth_note(cfg: config.Config) -> None:
    ...
    if auth.is_logged_in(agent):
        ...
        print(f"runtime not ready. run `choobi auth claude` or `choobi auth codex` "
              f"to pick your model and log in.")
